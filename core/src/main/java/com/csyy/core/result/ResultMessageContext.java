...
public class ResultMessageContext implements MessageContext {
    ...
    private final String MESSAGE_IS_NULL = "MESSAGE_IS_NULL";
    ...
    public void addMessageResourceFromStream(InputStream in) {
        ...
        for (Element resultMessage : XMLUtils.childrenElements(root, ELEMENT_NAME)) {
            logger.info("[key:{},code:{},message:{}]",
                    XMLUtils.getElementAttribute(resultMessage, ATTRIBUTE_KEY).getText(),
                    Integer.valueOf(XMLUtils.getElementAttribute(resultMessage, ATTRIBUTE_CODE).getText()),
                    XMLUtils.getElementAttribute(resultMessage, ATTRIBUTE_MESSAGE).getText());
            ResultInfo info = new ResultInfo(XMLUtils.getElementAttribute(resultMessage, ATTRIBUTE_KEY).getText(),
                    Integer.valueOf(XMLUtils.getElementAttribute(resultMessage, ATTRIBUTE_CODE).getText()),
                    XMLUtils.getElementAttribute(resultMessage, ATTRIBUTE_MESSAGE).getText());
            context.put(XMLUtils.getElementAttribute(resultMessage, ATTRIBUTE_KEY).getText(), info);
        }
    ...
    @Override
    public ResultInfo getResultInfo(String key, String... args) {
        ResultInfo resultInfo = getResultInfo(key);
        if (args == null || args.length <= 0) {
            logger.debug("自定义结果消息数组为空！");
            return resultInfo;
        }
        String result = resultInfo.getMessage();
        if (StringUtils.isEmpty(result))
            throw new CommonException(MESSAGE_IS_NULL);
        String message = MessageFormat.format(result, args);
        return new ResultInfo(key,resultInfo.getCode(),message);
    }
