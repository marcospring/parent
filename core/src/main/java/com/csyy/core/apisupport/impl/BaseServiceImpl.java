...
public class BaseServiceImpl implements BaseService {
    ...
    public <T> T id(Class<T> clazz, int id) {
        return factory.getCacheReadDataSession().querySingleResultById(clazz, id);
    }

    @Override
    public <T> T uuid(Class<T> clazz, String uuid) {
        return factory.getCacheReadDataSession().querySingleResultByUUID(clazz, uuid);
    }

    @Override
    public <T> T uuid(Class<T> clazz, String uuid, String exceptionKey) {
        T t;
        if ((t = uuid(clazz, uuid)) == null)
            throw new CommonException(exceptionKey);
        return t;
    }
}
