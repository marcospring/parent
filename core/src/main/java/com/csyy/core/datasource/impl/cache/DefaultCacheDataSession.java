...
import com.csyy.core.datasource.impl.common.DefaultDataSession;
import com.csyy.core.datasource.param.CustomSQL;
import com.csyy.core.datasource.param.ESQL;
...
import com.csyy.core.exception.DataAccessException;
import com.csyy.core.utils.CacheKeyUtils;
...
public class DefaultCacheDataSession extends DefaultDataSession implements CacheDataSession {
    ...
    private String systemKey = "DEFAULT";

    public DefaultCacheDataSession(String systemKey) {
        this.systemKey = systemKey;
    }


    public DefaultCacheDataSession(String systemKey, SqlSessionFactory sqlSessionFactory, RedisSessionFactory
            redisSessionFactory) {
        this.sqlSessionFactory = sqlSessionFactory;
        this.redisSessionFactory = redisSessionFactory;
        this.systemKey = systemKey == null || "".equals(systemKey) ? "DEFAULT" : systemKey;
    }
    ...
    public <T> T querySingleResultById(Class<T> clazz, int id) throws DataAccessException {
        ...
        try {
            ...
            if (redis == null) {
                ...
                String tableCacheKey = redis.getData(CacheKeyUtils.createTableKey(systemKey, clazz));
                String rowKey = CacheKeyUtils.createRowResultCacheKey(systemKey, tableCacheKey, String.valueOf(id), clazz
                        .getSimpleName());
    ...
    public <T> T querySingleResultByUUID(Class<T> clazz, String uuid) throws DataAccessException {
        ...
        try {
            ...
            if (redis == null) {
                ...
                String tableCacheKey = redis.getData(CacheKeyUtils.createTableKey(systemKey, clazz));
                String rowKey = CacheKeyUtils.createRowResultCacheKey(systemKey, tableCacheKey, uuid, clazz.getSimpleName
                        ());
                obj = redis.getTypeObject(clazz, rowKey);
                if (obj == null) {
                    obj = super.querySingleResultByUUID(clazz, uuid);
    ...
    public <T> List<T> queryListResult(Class<T> clazz, Param param) throws DataAccessException {
        ...
        try {
            ...
            logger.error("queryListResult:JSONException:{}", e);
            list = super.queryListResult(clazz, param);
        } catch (Exception e) {
            logger.error("cacheDao访问异常:{}", e);
            throw new DataAccessException(e);
...
//    }
    ...
    public <T> List<T> queryVOListByCustomElementName(Class<T> clazz, String elementId, Param param) throws DataAccessException {
        ...
        try {
            ...
            logger.error("queryVoListByCustomElementName:JSONException:{}", e);
            list = super.queryVOListByCustomElementName(clazz, elementId, param);
        } catch (Exception e) {
            logger.error("cacheDao访问异常:{}", e);
            throw new DataAccessException(e);
     ...
     * @param clazz
     */
    private <T> void updateTableKey(String systemKey, Class<T> clazz) {
        ...
        try {
            ...
            if (redis != null) {
                //更新表Key
                String cacheTableKey = CacheKeyUtils.createTableKey(systemKey, clazz);
    ...
    private void updateDatabaseKey() {
        ...
        try {
            ...
            if (redis != null) {
                ...
                redis.setData(systemKey + Constants.Separator.UPDERLINE + Constants.SQLConstants.DATABASE_KEY, databaseKey);
    ...
    private <T> String createTableResultKey(Class<T> clazz, ESQL esql, Map<String, Object> map, RedisSession redis) {
        try {
            ...
            String tableCacheKey = redis.getData(CacheKeyUtils.createTableKey(systemKey, clazz));
            return CacheKeyUtils.createTableResultCacheKey(systemKey, tableCacheKey, sql);
        } catch (RedisException e) {
    ...
    private String createDatabaseResultKey(Throwable able, String sqlId, Map<String, Object> map, RedisSession redis) {
        try {
            ...
            if (able != null) {
                ...
                String databaseCacheKey = redis.getData(systemKey + Constants.Separator.UPDERLINE + Constants.SQLConstants
                        .DATABASE_KEY);
                return CacheKeyUtils.createDatabaseResultCacheKey(systemKey, databaseCacheKey, sql);
            } else if (!StringUtils.isEmpty(sqlId)) {
                String sql = SQLUtil.getSql(getSession(), sqlId, map);
                String databaseCacheKey = redis.getData(systemKey + Constants.Separator.UPDERLINE + Constants.SQLConstants.DATABASE_KEY);
                return CacheKeyUtils.createDatabaseResultCacheKey(systemKey, databaseCacheKey, sql);
            }
        } catch (RedisException e) {
            //处理了RedisException,不向上抛e
