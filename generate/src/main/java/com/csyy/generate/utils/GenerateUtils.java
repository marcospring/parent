package com.csyy.generate.utils;


import com.csyy.common.StringDefaultValue;
import com.csyy.generate.db.MysqlConnection;
...
public class GenerateUtils {
	...
	public static String tableToPoName(String tableName) throws Exception{
		try {
			if(!StringDefaultValue.isEmpty(tableName)){
				//将表名翻译成PO名称
	...
	public static String columnNameToVariable(String columnName){
		if(!StringDefaultValue.isEmpty(columnName)){
	...
	public static String variableToColumnName(String variableName){
		if(!StringDefaultValue.isEmpty(variableName)){
			StringBuffer buff = new StringBuffer();
	...
	public static String dbTypeToVariableType(String dbType) throws Exception {
		if (StringDefaultValue.isEmpty(dbType)) {
			return null;
		}
	...
	public static String dbTypeToImportPath(String type){
		if(!StringDefaultValue.isEmpty(type) && DB_DATE_TYPE.indexOf(COMMA + type.toUpperCase() + COMMA) != -1
				){
			return IMPORT_DATE;
		}else if(!StringDefaultValue.isEmpty(type) && DB_DECIMAL_TYPE.indexOf(COMMA + type.toUpperCase() + COMMA) != -1){
