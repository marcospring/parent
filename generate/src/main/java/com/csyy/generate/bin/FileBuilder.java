...
public class FileBuilder {
    ...
    private void initFilePath() {
        ...
        if (!StringDefaultValue.isEmpty(config.getSystemName())) {
            String interfaceProjectRoot = config.getWorkspacesPath() + config.getProjectName() +
                    coommonJavaPath + config.getCompanyName() + Constants.Separator.SLASH +
                    config.getSystemName() + Constants.Separator.SLASH;
            //sqlmap文件路径
            filePathMap.put(SQLMAP_FILE_PATH, config.getWorkspacesPath() + config.getProjectName() + coommonResourcesPath + "sqlmap/po/" + config.getPoName() + Constants.FileType.XML_SUFFIX);
            //PO文件路径
